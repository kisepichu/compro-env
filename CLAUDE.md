実装は常に TDD を用いる。/spec-\* コマンドの手順に従う。
コミット前 `cargo test --all && cargo clippy --all --all-features -- -D warnings && cargo fmt --all --check`
...
2. **タスク生成・実装開始**: `/spec-do <command>` — タスクファイル生成 → TDD サイクルを回して実装
