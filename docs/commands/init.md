...
- 両側の変数名が一致する
- 少なくとも一方の添字ブレースが「複数部分」(複数のトークン、またはカンマ区切り) であること — 例 `{1W}`, `{iW}`, `{1,1}`
- 行全体がこのパターンのみで構成される (前後に他の変数がない)

検出された場合は `RawLine::GridRow` として分類し、ループボディの `loop_var_subscript` に行インデックス部分 (ブレース内の最初のトークン) を使用する。`var_type` は `Str` として直接付与し、制約テキストによる推定に依存しない。`flatten_single_var_loops` で `[String; H]` へフラット化される。
