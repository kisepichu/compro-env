...
input_format_raw (文字列、複数 pre ブロックは \n\n 区切りで結合済み)
  ...
  │    GridRow (文字グリッド行): 同名 Ident_{2D_left} [同名 Ident_{Num,Num}]* Cdots 同名 Ident_{2D_right}
  │      ※ 先頭要素が 1 個の既存形式 (S_{11} Cdots S_{1W}) も引き続き対象
  │      ※ 先頭に Ident_{Num,Num} が 2 個以上ある拡張形式 (S_{1,1} S_{1,2} \ldots S_{1,W}) も対象
  │      ※ cdots 前の全要素は名前・行インデックス(先頭サブスクリプト)が一致していること
  │
...
| 文字グリッド (2D 添字、先頭複数要素)                                       | `S_{1,1} S_{1,2} \ldots S_{1,W}` / `\vdots` / `S_{H,1} S_{H,2} \ldots S_{H,W}` | abc453-D           |
...
**文字グリッド (2D 添字) の検出**: `S_{11}...S_{1W}` や `S_{1,1} S_{1,2} \ldots S_{1,W}` のように `同名変数_{2D_left} [同名変数_{Num,Num}]* Cdots 同名変数_{2D_right}` の形をとる行は「グリッドの 1 行 = 1 つの `String`」として扱う。検出ルール:
...
- Cdots 前の追加要素 (`Ident_{Num,Num}` 形式) は行インデックス (先頭 Num) が全て一致していること
