...
input_format_raw (文字列、複数 pre ブロックは \n\n 区切りで結合済み)
  ...
  │      ※ 先頭に 2D 添字要素が 2 個以上ある拡張形式 (S_{1,1} S_{1,2} \ldots S_{1,W}) も対象
  │      ※ cdots 前の追加要素は名前・行インデックス (2D 添字の先頭パーツ) が全要素で一致していること
  │      ※ 追加要素の直前にはスペースが必要 (空白なし隣接 S_{1,1}S_{1,2} は ok: false フォールバック)
  │
...
- 追加要素の直前には少なくとも 1 つの Space が必要 (空白なし隣接は ok: false フォールバック)
