...
- 両側の変数名が一致する
- 少なくとも一方の添字ブレースが「2D」であること:複数トークン (`{1 W}`)、カンマ区切り (`{1,W}`)、または長さ 2 以上の単一トークン (`{H1}`, `{HW}`, `{iW}`) のいずれか
- 行全体がこのパターンのみで構成される (前後に他の変数がない)

検出された場合は `RawLine::GridRow` として分類し、ループボディの `loop_var_subscript` に行インデックス部分を使用する。行インデックスの抽出ルール: 複数トークン・カンマ区切りの場合は先頭パーツ、長さ ≥ 2 の単一トークン (`{H1}`, `{HW}`) の場合は先頭の 1 文字。`var_type` は `Str` として直接付与し、`all_int` 等の制約推定で上書きされない。`flatten_single_var_loops` で `[String; H]` へフラット化される。
